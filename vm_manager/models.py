    ...
    def save(self, *args, **kwargs):
        if not self.hostname_id:
            ...
            if hostname_id == ERROR:
                logger.error(f"Could not assign random value to volume "
                             f"{self.id} for user {self.user}")
                n = get_nectar()
                n.cinder.volumes.delete(self.id)
                raise ValueError("Could not assign random value to "
                                 "the volume")
...
class InstanceManager(models.Manager):
    ...
    def get_instance_by_untrusted_vm_id(self, vm_id, user,
        ...
        try:
            ...
            logger.error(f"Value error trying to get a VM with "
                         f"vm_id: {vm_id}, called by {user}")
            raise Http404
        except Instance.DoesNotExist:
            logger.error(f"Trying to get a vm that doesn't exist "
                         f"with vm_id: {vm_id}, called by {user}")
            raise Http404
        if instance.user != user:
            logger.error(f"Trying to get a vm that doesn't belong "
        ...
        if instance.boot_volume.requesting_feature != requesting_feature:
            logger.error(f"Trying to get a vm that doesn't belong "
                         f"to {requesting_feature} with vm_id: {vm_id}"
                         f"this vm belongs to "
                         f"{instance.boot_volume.requesting_feature}")
            raise Http404
        if instance.marked_for_deletion:
            if instance.deleted:
                logger.error(f"Trying to get a vm that has been deleted "
                             f"with vm_id: {vm_id}, called by {user}")
            else:
                logger.error(f"Trying to get a vm that is marked for "
...
class VMStatusManager(models.Manager):
    ...
    def get_vm_status_by_volume(self, volume, requesting_feature):
        ...
        try:
            ...
            logger.error(f"Trying to get_vm_status_by_volume {volume}, "
